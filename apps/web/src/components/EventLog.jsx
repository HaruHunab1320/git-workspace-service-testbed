    ...
    return (
      <div className="event-log card">
        ...
        <p className="event-log-empty">
          Advance the day to see events unfold...
        </p>
      </div>
    );
  }
    ...
    <div className="event-log card">
      ...
      <div className="event-log-scroll">
        {reports.map((report, i) => (
          <div
            key={i}
            className={`event-day ${report.is_magical ? 'magical-glow' : ''}`}
          >
            ...
            {report.festivals.length > 0 && (
              <div className="event-festival">
                🎉 {report.festivals.join(', ')}
              </div>
            )}
            {report.villager_events.slice(0, 3).map((e, j) => (
              <div key={j} className="event-line">
                👤 {e}
              </div>
            ))}
            ...
            {report.harvests.map((e, j) => (
              <div key={j} className="event-line event-harvest">
                🌾 {e}
              </div>
            ))}
            ...
            {report.found_items.map((e, j) => (
              <div key={j} className="event-line event-found">
                💠 {e}
              </div>
            ))}
          </div>
