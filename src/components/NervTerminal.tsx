...
export default function NervTerminal() {
  ...
  const processCommand = useCallback(
        ...
        case 'system': {
          if (flag === '--status') {
            outputLines.push(
              ...
              { type: 'output', text: `Sync Ratios     : ${JSON.stringify(store.syncRatios)}` },
    ...
    [store.emergencyLevel, store.syncRatios, store.magiVotes, appendLines],
  );
  ...
  return (
    <div
      className="flex flex-col bg-[#050505] border border-[#FF9900] font-mono text-sm h-full min-h-[300px]"
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-1 border-b border-[#FF9900] bg-[#050505]">
        ...
        <span className="text-[#FF3300] text-xs">
          MAGI UPLINK ● ACTIVE
        </span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="border-t border-[#FF9900] px-3 py-2 flex items-center gap-2">
        ...
        <input
          ...
          className="flex-1 bg-transparent text-[#FF3300] outline-none caret-[#FF3300] font-mono text-sm placeholder-[#FF3300]/30"
